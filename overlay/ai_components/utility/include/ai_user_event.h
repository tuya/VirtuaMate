 ...
 * @brief AI user event notification module header (VirtuaMate overlay)
 *
 * Extends upstream with AI_USER_EVT_EMOTION_TIMELINE for cloud timeIndex-synced
 * facial expressions on the VRM avatar.
 *
...
/**
 * @brief Cloud NLG emotion cue synchronized to TTS timeIndex (milliseconds).
 */
typedef struct {
    const char *name;
    uint32_t    time_ms;
    float       intensity;
} AI_NOTIFY_EMO_TIMELINE_T;
 ...
 * @param[in] cb Callback function pointer to be called when events occur
 * @return none
 */
 ...
 * @param[in] type Event type to notify
 * @param[in] data Pointer to event data (can be NULL)
 * @return none
 */
